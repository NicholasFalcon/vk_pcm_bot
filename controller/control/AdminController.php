    ...
    public function findAdminsByWebAction(): Response //TODO: нахуя?
    {
        ...
        if ($this->user->is_dev == 1 || $web->owner_id == $this->user->id) {
            foreach ($peers as $peer) {
                $AllUserChat = UserPeer::getAdminsKick($peer['id']);
                foreach ($AllUserChat as $admin) {
                    array_push($result, (int)$admin['user_id']);
                }
            ...
            if ($this->peer->getSetting(16))
                $response->message = "Вы не создатель данной сетки.";
        return $response;
    }

    public function ModuleAction($user_text, $object): Response
    {
        $response = new Response();
        $response->peer_id = $this->peer->id;
        $explode = explode(' ', $object['message']['text']);
        $explodes = mb_strtolower($explode[0]);
        if ($explodes == 'админка')
            $user_text = '1';
        if ($explodes == 'участник')
            $user_text = '2';
        if ($explodes == 'беседа')
            $user_text = '3';
        if ($explodes == 'сетка')
        ...
        if ($explodes == 'команды')
            $user_text = '5';
        if ($explodes == 'действия')
            $user_text = '6';
        if ($explodes == 'кланы')
            $user_text = '7';
        if ($explodes == 'игры')
            $user_text = '8';
        if ($explodes == 'техподдержка')
            $user_text = '';
        switch ($user_text) {
            case '1' :
            ...
            case '3' :
                $response->message = 'id - айди настройки, состояние - включено/выключено'
                    ...
                    . PHP_EOL . '11. Приветствие [text]'
                    . PHP_EOL . '12. Беседа мут [число]'
                    . PHP_EOL . '13. Беседа -мут';
                $response->setButtonRow(['Беседа инициализация', '1'], ['Беседа обновить', '2']);
                $response->setButtonRow(['Беседа настройки', '3'], ['Чатссылка', '4']);
                $response->setButtonRow(['Кик собак', '5'], ['Неактив Кик', '6']);
...
//                    . PHP_EOL . "3. Загадки"
//                    . PHP_EOL . "3.1 Игра старт загадки"
//                    . PHP_EOL . "3.2 Слово 'нужное слово'"
//                    . PHP_EOL . "3.3 Игра стоп загадки"
...
//                    . PHP_EOL . "2. [eoremic|Антон] (В сети почти всегда, middle bot developer)"
                ;
                $response->setButtonRow(['Модуль 1', '1'], ['Модуль 2', '2']);
                $response->setButtonRow(['Модуль 3', '3'], ['Модуль 4', '4']);
                $response->setButtonRow(['Модуль 5', '5'], ['Модуль 6', '6']);
                $response->setButtonRow(['Модуль 7', '7'], ['Модуль 8', '8']);
                break;
        }
        return $response;
    }
        ...
        if ($this->userPeer->haveAccess(Role::BAN_ACCESS) || $this->user->is_dev == 1) {
            if ($user !== false) {
                if ($user->isExists()) {
                    ...
                    $this->peer->save();
                    $admins = UserPeer::getAdminsKick($this->peer->id);
                    $message = $this->render('admin/kick_admin', [
                        'admins' => $admins
                    ]);
                    $this->vk->messagesSend($this->peer->id, 'Пользователь забанен в беседе. Если вы не согласны напишите админам: '
                        . PHP_EOL . $message);
                    $result = $this->vk->messagesRemoveChatUser($this->peer->id, $userPeer->user_id);
    ...
    public function kickAction($object, $user_text): Response
    {
        ...
        if ($this->userPeer->haveAccess(Role::KICK_ACCESS) || $this->user->is_dev == 1) {
            if ($user !== false) {
                ...
                $this->vk->messagesSend($this->peer->id, 'Пользователь удален из беседы. Если вы не согласны напишите админам: '
                    . PHP_EOL . $message);
                $result = $this->vk->messagesRemoveChatUser($this->peer->id, $userPeer->user_id);
                if (isset($result['error']) && $result['error']['error_code'] == 15) {
                    $response->message = 'Не могу выгнать админа';
...
//    }
    ...
    public function getOnlineAction($user_text): Response
    {
        $response = new Response();
        $response->peer_id = $this->peer->id;
        if ($user_text == '') {
...
//                            $this->user->createNotification("[id{$userPeer->user_id}|Пользователь] был размучен по истечению срока. Впредь не хулиганьте.", $this->peer->id, time() + intval($user_text));
    ...
    public function removeWarningAction($object, $user_text): Response
    {
        ...
        $user = $this->getUserFromMessage($user_text, $object);
        if ($user->id != $this->user->id) {
            $response->message = 'Вы не можете снять предупреждение с Вас!';
            return $response;
        }
        if ($this->userPeer->haveAccess(Role::PRED_ACCESS) || ($this->user->is_dev == 1)) {
            $userPeer = UserPeer::findsByPeerAndUser($user->id, $this->peer->id);
            ...
            if ($warning->isExists()) {
                ...
                $response->message = 'У вас нет предупреждений.';
            }
        } else {
            $response->message = "Вам не доступна данная команда!";
        }
    ...
    public function allWarningAction($user_text): Response
    {
        $response = new Response();
        $response->peer_id = $this->peer->id;
        if ($user_text == '') {
    ...
    public function chatUrlAction($user_text): Response
    {
        $response = new Response();
        $response->peer_id = $this->peer->id;
    ...
    public function sleepersAction($user_text): Response
    {
        $response = new Response();
        $response->peer_id = $this->peer->id;
        if ($user_text == '')
            if ($this->userPeer->is_admin == 1 || ($this->user->is_dev == 1)) {
                $userInfo = UserPeer::getSleepersUsers($this->peer->id);
...
//        $response->message = print_r($text,1);
    ...
    public function KickDeactivatedAction(): Response
    {
        ...
        if ($this->user->is_dev == 1 || $this->userPeer->haveAccess(Role::KICK_ACCESS)) {
            ...
            $this->vk->messagesSend($this->peer->id, "Удаляю собачек...");
            ...
            $this->vk->messagesSend($this->peer->id, "Сделано!");
        } else {
            $response->message = "Вам не доступна данная команда!";
        }
        return $response;
    ...
    public function KickLeaversAction(): Response
    {
        $response = new Response();
        $response->peer_id = $this->peer->id;
        //|| $this->userPeer->status >= $this->peer->getSetting(3)
        if ($this->user->is_dev == 1 || $this->userPeer->haveAccess(Role::KICK_ACCESS)) {
            $allUser = [];
            $items = [];
            $this->vk->messagesSend($this->peer->id, "Кикаю вышедших из беседы...");
            $users = UserPeer::SelectUsers($this->peer->id);
            $result = $this->vk->messagesGetConversationMembers($this->peer->id);
            ...
            $this->vk->messagesSend($this->peer->id, "Чистка окончена....");
        } else {
            $response->message = "Вам не доступна данная команда!";
        }
        return $response;
