...
class PeerController extends Controller
{
    ...
    public function attentionAction()
    {
        ...
        if ($user->check < $this->peer->getSetting(App::S_COUNT_ATTENTION) && $user->role_id == Role::MAIN_ADMIN && $this->user->is_dev == 1) {
    ...
    public function autoKickOnAction()
    {
        ...
        if ($this->userPeer->role_id == Role::MAIN_ADMIN || $this->user->is_dev == 1) {
            if ($this->peer->getSetting(App::S_INACTIVE_KICK) == 1) {
                $this->peer->setSetting(App::S_INACTIVE_KICK, 1);
                $this->peer->save();
                $response->message = 'Кик пользователей при их выходе из беседы включен.';
            } else
                $response->message = "Кик пользователей при их выходе из беседы уже включен.";
        }  else
            $response->message = "Вы не имеете доступ к данной команде.";
        return $response;
    }
    ...
    public function autoKickOffAction()
    {
        ...
        $response->peer_id = $this->peer->id;
        if ($this->userPeer->role_id == Role::MAIN_ADMIN || $this->user->is_dev == 1) {
            if ($this->peer->getSetting(App::S_INACTIVE_KICK) == 0) {
            $this->peer->setSetting(App::S_INACTIVE_KICK, 0);
            $this->peer->save();
            $response->message = 'Кик пользователей при их выходе из беседы выключен.';
            } else
                $response->message = "Кик пользователей при их выходе из беседы уже выключен.";
        } else
            $response->message = "Вы не имеете доступ к данной команде.";
        return $response;
    }
